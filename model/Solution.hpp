...
public:
	...
	void sort_cells_by_coord() {
		...
		for (unsigned i = 0; i < values_at_cells.size(); i++) {
			...
			Point<dim> bottom_left_dealii = values_at_cells[i].first->vertex(0);
			Point<dim> top_right_dealii = values_at_cells[i].first->vertex(
					vertices_per_cell - 1);
			...
			rtree.insert(std::make_pair(rect, values_at_cells[i].second));
		}
	}
...
//				return found_cell;
//			}
	/*std::pair<typename DoFHandler<dim>::active_cell_iterator, ValuesAtCell<dim>> get_cell(
			Point<dim> const &point) {

		for (unsigned i = 0; i < values_at_cells.size(); i++) {

			int vertices_per_cell = GeometryInfo<dim>::vertices_per_cell;
			Point<dim> top_right = values_at_cells[i].first->vertex(
					vertices_per_cell - 1);
	...
	PhysicalValues<dim> extrapolate_values(Point<dim> const &point) {
		...
		rtree.query(bgi::intersects(query_point), std::back_inserter(result_s));
		//std::cout << "after query" << std::endl;
		value values_at_cell = result_s[0];
		//std::cout << "after getting values at cell from vector" << std::endl;
		...
		//std::cout << "before getting coord" << std::endl;
