...
void test_case_silicium(){
	...
	double refine_accuracy = potential/100.0;
	unsigned ec_refine_level = 7;

	SerratedRect2DDetector srdd(nbr_of_strips, width, strip_length, strip_width,
			half_pitch, potential, refine_accuracy, max_iter, stop_accuracy, TYPE_SILICIUM);
	...
	srdd.draw_vtk_graph_weight_potential(output_dir +
			"silicium_weight_potential.vtk");
	...
	ResultsOut::write_current_vs_time("../src/plots/silicium_current_vs_time.txt",
				current_vs_time);
}
