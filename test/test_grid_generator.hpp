		...
		for (unsigned width = 100; width <= 300; width += 100) {
			for (unsigned holes_radius = 10; holes_radius <= 30; holes_radius +=
				...
				for (unsigned inter_holes_centers_dist = 3*holes_radius;
					...
					std::string output_file = "nbr_of_holes_" + std::to_string(nbr_of_holes)
											+ "_width_ " + std::to_string(width) + "_holes_radius_" + std::to_string(holes_radius)
											+ "_inter_holes_centers_dist_" + std::to_string(inter_holes_centers_dist)+".eps";

					try{
						Triangulation<2> tria;
						MyGridGenerator<2>::rectangle_with_circular_holes(tria, width,
								holes_radius, inter_holes_centers_dist, nbr_of_holes);
						GridOut grid_out;
						...
						std::cout << output_file << " throws exception " << e << std::endl;
					}
				}
